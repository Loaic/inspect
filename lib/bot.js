...
class Bot extends EventEmitter {
    ...
    constructor(settings) {
        ...
        this.busy = false;
        this.eventHandlersBound = false; // Flag to prevent duplicate event binding
        ...
        this.lastSteamActivity = Date.now();
        this.healthCheckInterval = setInterval(() => {
            this.performHealthCheck();
        }, 2 * 60 * 1000); // Check every 2 minutes (less aggressive)
    }
    ...
    async logIn(username, password, auth) {
        ...
        if (arguments.length === 3) {
            ...
            // Only bind event handlers once during initial setup
            this.bindEventHandlers();
        }
    ...
    bindEventHandlers() {
        // Prevent duplicate event handler binding
        if (this.eventHandlersBound) {
            winston.debug(`${this.username} Event handlers already bound, skipping`);
            return;
        }
        
        winston.debug(`${this.username} Binding event handlers`);
        ...
        this.steamClient.on('disconnected', (eresult, msg) => {
            ...
            // Cancel any pending GC reconnection since Steam is disconnected
        ...
        this.csgoClient.on('disconnectedFromGC', (reason) => {
            ...
            // Only schedule reconnection if we're not already trying to reconnect
            // and if Steam is still logged in
            if (!this.gcReconnectTimeout && this.steamClient.loggedOn) {
        ...
        this.csgoClient.on('connectionStatus', (status) => {
            ...
            // Update activity timestamp on connection status changes
            this.lastGCActivity = Date.now();
        });
    ...
    performHealthCheck() {
        ...
        const steamInactiveTime = now - this.lastSteamActivity;
        const maxGCInactiveTime = 20 * 60 * 1000; // 20 minutes (more lenient)
        const maxSteamInactiveTime = 15 * 60 * 1000; // 15 minutes

        // Skip health check if we're already in the middle of reconnecting
        if (this.loginRetryTimeout || this.gcReconnectTimeout) {
            winston.debug(`${this.username} Health check: Skipping - reconnection in progress`);
            return;
        }

        // Check if Steam has been inactive for too long
        if (this.steamClient.loggedOn && steamInactiveTime > maxSteamInactiveTime) {
            winston.warn(`${this.username} Health check: Steam inactive for ${Math.round(steamInactiveTime / 1000)}s, forcing reconnection`);
            this.steamClient.relog();
            return;
        }
        ...
        // Only check GC inactivity if bot should be processing requests
        // Skip GC inactivity check if there's no recent activity requirement
        if (this.ready && this.busy === false && gcInactiveTime > maxGCInactiveTime) {
        ...
        // Check if we should be ready but aren't (only if Steam is connected and stable)
        if (this.steamClient.loggedOn && !this.ready && steamInactiveTime < 5 * 60 * 1000) {
            winston.warn(`${this.username} Health check: Should be ready but isn't, attempting GC reconnection`);
            this.scheduleGCReconnect();
            return;
        }

        winston.debug(`${this.username} Health check: OK (Steam: ${this.steamClient.loggedOn ? 'connected' : 'disconnected'}, GC: ${this.ready ? 'ready' : 'not ready'}, GC inactive: ${Math.round(gcInactiveTime / 1000)}s)`);
    }



    /**
     * Clean up timers and event handlers when bot is destroyed
     */
    destroy() {
        winston.info(`${this.username} Destroying bot and cleaning up resources`);
        ...
        if (this.csgoClient) {
            this.csgoClient.removeAllListeners();
        }
