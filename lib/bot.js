...
class Bot extends EventEmitter {
    ...
    async logIn(username, password, auth) {
        ...
        // Get random proxy if proxy manager is available
        if (this.proxyManager) {
            try {
                const proxyResult = await this.proxyManager.getRandomProxy();
                if (proxyResult) {
                    this.currentProxyConfig = proxyResult.config;
                    winston.info(`${this.username} Using proxy: ${proxyResult.name}`);
                    
                    // Update steam client with new proxy configuration
                    this.steamClient = new SteamUser(Object.assign({
                        promptSteamGuardCode: false,
                        enablePicsCache: true,
                        httpProxy: proxyResult.config.httpProxy
                    }, this.settings.steam_user));
                    
                    this.csgoClient = new GlobalOffensive(this.steamClient);
                    this.bindEventHandlers();
                } else {
                    winston.warn(`${this.username} Failed to get proxy, using direct connection`);
                }
            } catch (error) {
