...
class Bot extends EventEmitter {
    ...
    async setupProxyConnection() {
        try {
            ...
            // 如果启用了外部代理，直接使用配置的代理
            if (this.settings.external_proxy && this.settings.external_proxy.enabled) {
                const proxyConfig = this.settings.external_proxy;
                const httpPort = proxyConfig.http_port || 20171;
                const socksPort = proxyConfig.socks_port || 20170;
                const proxyName = proxyConfig.proxy_name || 'external-proxy';

                // 设置代理配置
                steamOptions.httpProxy = `http://127.0.0.1:${httpPort}`;
                steamOptions.socksProxy = `socks5://127.0.0.1:${socksPort}`;
                
                this.currentProxyConfig = {
                    httpProxy: steamOptions.httpProxy,
                    socksProxy: steamOptions.socksProxy,
                    proxyName: proxyName
                };

                winston.info(`${this.username} Using external proxy: ${proxyName} (HTTP: ${httpPort}, SOCKS: ${socksPort})`);
            } else {
                winston.info(`${this.username} Using direct connection (no proxy configured)`);
            }
