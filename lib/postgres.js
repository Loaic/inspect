...
class Postgres {
    ...
    createPool() {
        if (this.pool) {
            ...
            this.pool.end().catch(err => winston.warn('Error closing old pool:', err));
        }
        ...
        this.pool = new Pool({
            ...
            application_name: 'csgofloat_api',
            keepAlive: true,
            keepAliveInitialDelayMillis: 10000,
        });

        this.setupPoolEventHandlers();
        winston.info('PostgreSQL connection pool created');
    }
    ...
    setupPoolEventHandlers() {
        ...
        this.pool.on('connect', (client) => {
            winston.debug('New client connected to PostgreSQL pool');
        });

        this.pool.on('acquire', (client) => {
            winston.debug('Client acquired from PostgreSQL pool');
        });

        this.pool.on('remove', (client) => {
            winston.debug('Client removed from PostgreSQL pool');
        });
    }
    ...
    async handlePoolError(err) {
        ...
        winston.warn('Pool error detected, attempting to recreate connection pool');
        
        try {
            await new Promise(resolve => setTimeout(resolve, 5000)); // Wait 5 seconds before reconnecting
            this.createPool();
            winston.info('PostgreSQL pool successfully recreated');
        } catch (reconnectErr) {
            winston.error('Failed to recreate PostgreSQL pool:', reconnectErr);
        } finally {
            this.isReconnecting = false;
        }
    }
    ...
    async executeQuery(query, params = [], retryCount = 0) {
        try {
            ...
            winston.warn(`Query failed (attempt ${retryCount + 1}):`, err.message);
            
            // Check if error is connection-related and we haven't exceeded max retries
            if (this.isConnectionError(err) && retryCount < this.maxRetries) {
                const delay = this.retryDelay * Math.pow(2, retryCount); // Exponential backoff
                winston.info(`Retrying query in ${delay}ms...`);
    ...
    isConnectionError(err) {
        ...
        return connectionErrorCodes.some(code => 
            err.code === code || err.message?.includes(code) || err.toString?.().includes(code)
        );
    }
    ...
    async connect() {
        try {
            ...
            winston.info('PostgreSQL connection established and schema verified');
        } catch (err) {
            winston.error('Failed to connect to PostgreSQL:', err);
            throw err;
        }
    }
    ...
    async ensureSchema() {
        await this.executeQuery(`CREATE TABLE IF NOT EXISTS items (
            ms          bigint  NOT NULL,
            a           bigint  NOT NULL,
            d           bigint  NOT NULL,
        ...
        await this.executeQuery(`CREATE INDEX IF NOT EXISTS i_stickers ON items USING gin (stickers jsonb_path_ops) 
                                WHERE stickers IS NOT NULL`);
        await this.executeQuery(`CREATE INDEX IF NOT EXISTS i_paintwear ON items (paintwear)`);
        await this.executeQuery(`CREATE UNIQUE INDEX IF NOT EXISTS i_unique_item ON 
                                items (defindex, paintindex, paintwear, paintseed)`);
        await this.executeQuery(`CREATE UNIQUE INDEX IF NOT EXISTS i_unique_fid ON items (floatid)`);
    }
    ...
    async handleBulkInsert(data) {
        ...
        try {
            ...
            await this.executeQuery(query, flatten(values));
            winston.debug(`Inserted/updated ${values.length} items`)
        } catch (e) {
            winston.warn('Bulk insert failed:', e);
        }
    }
    ...
    updateItemPrice(assetId, price) {
        return this.executeQuery(`UPDATE items SET price = $1 WHERE a = $2`, [price, assetId]);
    }
    ...
    getItemRank(id) {
        return this.executeQuery(`SELECT (SELECT Count(*)+1
                                        FROM   (SELECT * 
                                                FROM   items T 
                                                WHERE  T.paintwear < S.paintwear 
                                                       AND T.defindex = S.defindex 
    ...
    async close() {
        try {
            if (this.pool) {
                winston.info('Closing PostgreSQL connection pool...');
                await this.pool.end();
                winston.info('PostgreSQL connection pool closed');
            }
        } catch (err) {
            winston.error('Error closing PostgreSQL pool:', err);
            throw err;
        }
    }
}
